...
architecture RTL of Packet_FSM_Decoder is
  ...
  type t_SM_Main is (S_WAIT_START, S_READ_ID, S_WAIT_COLON, S_READ_VALUE);
    ...
    if rising_edge(i_Clk) then
      ...
      if i_Reset_POR = '1' then
        ...
        case r_SM_Main is
          ...
          when S_READ_VALUE =>
  if i_rx_char = C_ASCII_HASH then
    -- 1. Latch data immediately when '#' is detected
    case r_active_sensor is
      when ID_AQI   => r_AQI_reg  <= std_logic_vector(r_current_value);
      when ID_TEMP  => r_TEMP_reg <= std_logic_vector(r_current_value);
      when ID_LIGHT => r_LIGHT_reg <= std_logic_vector(r_current_value);
      when others   => null;
    end case;
    ...
    if r_active_sensor = ID_LIGHT then
      ...
      -- Move to READ_ID to wait for the NEXT character (T or L)
      r_SM_Main     <= S_READ_ID;
    end if;

  elsif w_current_digit <= 9 then
    v_next_value := (r_current_value sll 3) + (r_current_value sll 1) + 
